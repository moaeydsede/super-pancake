...
const أدمن_UID = 'JxKXouwjdadht4wSMPf1qtbeW9n1';
...
const BASE_مستخدم_PERMISSIONS = Object.fromEntries(permissionsCatalog.map((item) => [item.key, false]));
...
function normalizePermissions(user = {}) {
  return user.role === 'admin' ? { ...ALL_PERMISSIONS } : { ...BASE_مستخدم_PERMISSIONS, ...(user.permissions || {}) };
}
...
function normalizeData(input) {
  ...
  data.users = data.users.map((user, index) => ({
    ...
    email: user.email || '',
    role: user.id === أدمن_UID || user.uid === أدمن_UID || user.role === 'admin' ? 'admin' : 'user',
    status: ['active', 'pending', 'suspended'].includes(user.status) ? user.status : 'active',
    permissions: normalizePermissions(user.id === أدمن_UID || user.uid === أدمن_UID ? { ...user, role: 'admin' } : user),
  ...
  profileUnsub = onSnapshot(
    ...
    async (snapshot) => {
      try {
        ...
        if (!profileData && authUser.uid === أدمن_UID) {
...
function makeUserForm(item = null) {
  return {
    ...
    permissions: item?.role === 'admin' ? { ...ALL_PERMISSIONS } : { ...BASE_مستخدم_PERMISSIONS, ...(item?.permissions || { materials: true }) },
  };
}
  ...
  return `
    <div class="auth-shell">
      <section class="auth-visual luxury-card">
        <div class="auth-badge">ERP PRO CLOUD</div>
    ...
    <div class="auth-shell">
      ...
      <section class="auth-panel glass-card">
        ...
        <form id="login-form" class="form-grid auth-form single-column-form">
          ...
          <div class="auth-note">مزامنة فورية • قاعدة بيانات سحابية • تثبيت على الجوال</div>
        </form>
      </section>
    </div>
  `;
    ...
    <aside class="sidebar glass-card ${state.ui.mobileMenuOpen ? 'open' : ''}">
      ...
      <div class="brand-block">
        ...
        <div>
          ...
          <div class="brand-subtitle">منصة إدارة احترافية</div>
        </div>
      </div>
      <div class="profile-card">
        ...
        <div>
          ...
          <div class="profile-role">${user?.role === 'admin' ? 'مدير النظام' : escapeHtml(user?.username || '')}</div>
        </div>
      </div>
      <nav class="nav-list">
        ${navItems
      ...
      <div class="sidebar-footer">
        ...
        <div class="mini-kpi"><span>التثبيت</span><strong>${state.ui.installed ? 'مثبّت' : 'وضع المتصفح'}</strong></div>
      </div>
    </aside>
  `;
}
  ...
  return `
    <header class="topbar glass-card">
      ...
      <div class="topbar-actions full-width-mobile">
        ...
        <div class="status-pill connected">${state.session?.role === 'admin' ? 'أدمن' : 'مستخدم'}</div>
        <div class="status-pill demo">المخزون ${summary.totalStock}</div>
  ...
  return `
    <div class="dashboard-grid">
      <section class="hero-banner luxury-card">
        <div class="hero-copy">
          <span class="hero-tag">لوحة التحكم التنفيذية</span>
      ...
      <section class="glass-card sticky-panel">
        ...
        <form id="user-form" class="form-grid">
          ...
          <label><span>الدور</span>
            <select name="role" data-bind="user.role" ${editing?.id === أدمن_UID ? 'disabled' : ''}>
          ...
          <label><span>الحالة</span>
            <select name="status" data-bind="user.status" ${editing?.id === أدمن_UID ? 'disabled' : ''}>
          ...
          <div class="full permission-grid">
            ${permissionsCatalog
              .map(
                (perm) => `
                  <label class="permission-chip ${state.forms.user.permissions[perm.key] ? 'on' : ''} ${state.forms.user.role === 'admin' || editing?.id === أدمن_UID ? 'disabled' : ''}">
                    <input type="checkbox" data-permission="${perm.key}" ${checked(state.forms.user.permissions[perm.key])} ${state.forms.user.role === 'admin' || editing?.id === أدمن_UID ? 'disabled' : ''} />
        ...
        ${renderTable(
          ...
          state.data.users.map((user) => [
            ...
            Object.entries(user.permissions || {})
              ...
              .join(' • '),
            `<div class="table-actions"><button class="ghost-mini-btn" data-action="edit-user" data-id="${escapeHtml(user.id)}">تعديل</button>${user.id !== state.session?.id && user.id !== أدمن_UID ? `<button class="danger-mini-btn" data-action="delete-user" data-id="${escapeHtml(user.id)}">تعطيل</button>` : ''}</div>`,
          ]),
        )}
      </section>
    </div>
...
async function submitUser(event) {
  ...
  const payload = {
    ...
    role: form.role,
    status: form.status,
    permissions: form.role === 'admin' ? { ...ALL_PERMISSIONS } : { ...BASE_مستخدم_PERMISSIONS, ...form.permissions },
    createdAt: previous?.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
...
async function deleteUser(id) {
  if (id === state.session?.id || id === أدمن_UID) {
  ...
  try {
    await setDoc(doc(db, 'users', id), {
      ...
      role: 'user',
      status: 'suspended',
      permissions: { ...BASE_مستخدم_PERMISSIONS },
      createdAt: user.createdAt || new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    }, { merge: true });
...
function handleChange(event) {
  ...
  if (event.target.dataset.bind === 'user.role') {
    ...
    state.forms.user.permissions = event.target.value === 'admin' ? { ...ALL_PERMISSIONS } : { ...BASE_مستخدم_PERMISSIONS, ...state.forms.user.permissions };
    render();
    return;
  }
